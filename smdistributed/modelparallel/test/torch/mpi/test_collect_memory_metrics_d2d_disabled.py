...
class TestStepMemoryMetricsCollectorD2DDisabled(unittest.TestCase):
    """
    Class to test StepMemoryMetricsCollector when d2d is disabled
    Since the current d2d caches the settings through multiple runs,
    so we have to create a separate test file to test different settings.
    """

    def test_simple_step_memory_metrics_collect_with_d2d_disabled(self):
        # Disable D2D
        os.environ["SMP_DISABLE_D2D"] = "1"
        ...
        self.assertRegex(
            ...
            r"gpu_total_memory_mb \d+ smp_backend_d2d_peak_allocated_mb 0 smp_backend_d2d_peak_reserved_mb 0 ",
        )
