...
from tensorflow.core.protobuf import meta_graph_pb2, saved_model_pb2

# First Party
from smdistributed.modelparallel.tensorflow.graph_def_editor import node, tensor, util, variable

if sys.version >= "3":
    from typing import Tuple, Dict, FrozenSet, Iterable, Union, Set, Any


__all__ = ["Graph", "SaverInfo", "SignatureInfo", "GraphVisitor", "saved_model_to_graph"]
  ...
  * _collections: Map from collection name to collection contents for all
  ...
  """
    ...
    """
        if g is None:
        ...
        elif isinstance(g, graph_pb2.GraphDef):
            graph_def = g
            ...
            raise TypeError(
                "Graph is of type {}. Expected a tf.Graph or GraphDef " "proto".format(type(g))
            )
        if name is None:
        ...
        for node_def in graph_def.node:
            self[node_def.name].set_inputs_from_strings(node_def.input, set_control_inputs=True)
    ...
    def add_collection_from_collection_def(
        self, collection_name, collection_def, validate_name=True
    ):
    ...
    """
        ...
        if collection_def.HasField("node_list"):
            ...
            for serialized_var in collection_def.bytes_list.value:
                var = self.add_variable_from_variable_def(serialized_var, skip_if_present=True)
                var.add_to_collection(collection_name)
        elif (
            collection_def.HasField("int64_list")
    ...
    """
        if not isinstance(name, string_types):
            raise ValueError(
                "Node name argument is not a string, but is of type " "{}".format(type(name))
            )
        return name in self._node_name_to_node
    ...
    """
        if uniquify_name:
            ...
            raise ValueError(
                "Graph already contains a node with name '{}' "
                "(Note that this check is case-insensitive).".format(name)
            )
    ...
    """
        ...
        if set_inputs:
            ret.set_inputs_from_strings(node_def.input, set_control_inputs=set_control_inputs)
        ret.device = node_def.device
        ret.clear_attrs()
        for key in node_def.attr:
        ...
        if check_for_refs:
            for t in n.outputs:
                if len(t.consumers()) > 0:
                    raise ValueError(
                        "Removing node '{}' would leave dangling "
                        "references from nodes {} to tensor '{}'"
                        "".format(name, [c.name for c in t.consumers()], t.name)
                    )
    ...
    """
        if self.contains_node(new_name):
            raise ValueError("Graph already has a node under name '{}'".format(new_name))
    ...
    def add_variable_from_variable_def(self, variable_def, skip_if_present=False):
        # type: (Any, bool) -> None
        """
    ...
    """
        ...
        if os.path.exists(saved_model_path):
            raise ValueError("Output path '{}' already exists".format(saved_model_path))
        if not os.path.exists(os.path.dirname(saved_model_path)):
            raise ValueError(
                "Parent directory '{}' of output dir '{}' does not "
                "exist".format(os.path.dirname(saved_model_path), saved_model_path)
            )
        ...
        if self.has_passthrough_saver:
            ...
            dir_util.copy_tree(
                self._passthrough_saver.path, _vars_dir_for_saved_model(saved_model_path)
            )
        ...
        for coll_name, coll_type in self._collection_name_to_type.items():
            if coll_type == "passthrough":
                meta_graph.collection_def[coll_name] = self._passthrough_collections[coll_name]
                ...
                meta_graph.collection_def[coll_name].bytes_list.value.extend(serialized_vars)
            elif coll_type == "node":
        ...
        def _add(names, type_name):
            for coll_name in names:
                if coll_name in self._collection_name_to_type:
                    raise ValueError(
                        (
                            _duplicate_collection_error_str(
                                coll_name,
    ...
    *A word about frames*
    ...
    def infer_shapes_and_dtypes(
        self, starting_nodes=None  # type: Iterable[node.Node]
    ):
        # type: (...) -> None
        """
    Visit all nodes reachable from a starting set in the order of a
            ...
            def visit_node(
                ...
                if cur_node.op_type in ["Enter", "RefEnter"]:
                    ...
                    if _FRAME_NAME_ATTR not in cur_node.get_attr_keys():
                        raise ValueError(
                            "Node {} is of op type {} but does not have a "
                            "value for its {}"
                            " attribute".format(cur_node.name, cur_node.op_type, _FRAME_NAME_ATTR)
                        )
...
def saved_model_to_graph(
  ...
  """
    if not os.path.exists(saved_model_path):
        raise ValueError("SavedModel root directory {} not found".format(saved_model_path))
    if not os.path.isdir(saved_model_path):
        raise ValueError("SavedModel root path {} is not a directory".format(saved_model_path))
    ...
    if tag is None:
        if len(saved_model.meta_graphs) != 1:
            raise ValueError(
                "No tags specified and there are multiple "
        ...
        matching_ixs = [
            i
            for i in range(len(saved_model.meta_graphs))
            if tag in saved_model.meta_graphs[i].meta_info_def.tags
        ]
        if len(matching_ixs) == 0:
            raise ValueError(
                "No MetaGraphDef in SavedModel at {} contains tag "
                "'{}'".format(saved_model_path, tag)
            )
        if len(matching_ixs) > 1:
            raise ValueError(
                "{} different MetaGraphDef in SavedModel at {} "
    ...
    if include_saver and meta_graph.HasField("saver_def"):
        saver_info = SaverInfo(_vars_dir_for_saved_model(saved_model_path), meta_graph.saver_def)
    else:
        saver_info = None
    signature_info = SignatureInfo()
...
def _decode_graph(graph_def):
  ...
  """
    ...
    #   try:
    #       shape = n.attr['shape']
    #       dtype = tf.as_dtype(n.attr['shape'])
    #   except ValueError:
    #       output_map[n.name] = []
    ...
    output_map = {
        op.name: [(t.dtype, t.shape) for t in op.outputs] for op in temp_graph.get_operations()
    }
    return output_map
...
def _extract_collection_defs(meta_graph):
    ...
    for collection_name in meta_graph.collection_def:
        if type(collection_name) is not str:
            ...
            continue
        elif collection_name in ("while_context", "cond_context", "savers", "queue_runners"):
...
def _duplicate_collection_error_str(
  ...
  """
    ...
    if name in node_collection_names:
        types.append("node")
    return "Collection name '{}' maps to multiple collection types: " "{}".format(name, types)
