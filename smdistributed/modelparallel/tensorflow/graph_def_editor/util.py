...
# First Party
from smdistributed.modelparallel.tensorflow.graph_def_editor import graph, node, tensor
...
def make_placeholder_from_tensor(
  ...
  """
    return make_placeholder(
        g, dtype=t.dtype, shape=t.shape, name=name
    )  # placeholder_name(t, scope=scope,
    #           prefix=prefix))
...
def make_placeholder_from_dtype_and_shape(g, dtype, shape=None, scope=None):  # ,
  ...
  """
    return make_placeholder(
        g, dtype=dtype, shape=shape, name=name
    )  # placeholder_name(scope=scope, prefix=prefix))
...
def get_predefined_collection_names():
    ...
    return [
        getattr(tf.GraphKeys, key)
        for key in dir(tf.GraphKeys)
...
def _python_type_to_attr_list_elem(
    list_value,  # type: attr_value_pb2.AttrValue.ListValue
    elem,  # type: Any
):
    """
  Subroutine of python_type_to_attr_value(). Converts one element of a Python
  list to one element of a `attr_value_pb2.AttrValue.ListValue` protobuf.
  ...
  """
    ...
    # TODO(frreiss): Populate the "func" field of the union here
        ...
        raise ValueError(
            "Don't know how to convert a {} to "
            "attr_value_pb2.AttrValue.ListValue".format(type(elem))
        )
...
def python_type_to_attr_value(
    ...
    # type (...) -> attr_value_pb2.AttrValue
    """
  Convert a Python object or scalar value to a TensorFlow `attr_value_pb2.AttrValue`
  ...
  """
    if isinstance(value, list) or isinstance(value, tuple):
        if 0 == len(value):
            return attr_value_pb2.AttrValue(list=attr_value_pb2.AttrValue.ListValue())
        else:
            # Nonempty list
    ...
    # Must check for bool before int because bool is a subclass of int in Python
        ...
        return attr_value_pb2.AttrValue(tensor=tf.make_tensor_proto(values=value))
    # TODO(frreiss): Populate the "func" and "placeholder" fields of the union
    #  here
    else:
        raise ValueError(
            "Don't know how to convert {} to " "attr_value_pb2.AttrValue".format(value)
        )


def attr_value_to_python_type(
    attr_value  # type: attr_value_pb2.AttrValue
):
    # type (...) -> Any
    """
  ...
  """
    ...
    #  print('lsttt', attr_value.list.i)
    #  print('lsttt', attr_value.list.s)
    #  print('lsttt', attr_value.list.b)
    #  print('lsttt', attr_value.list.f)
    #  return out
    ...
    #  here
        ...
        raise ValueError(
            "Don't know how to convert AttrValue {} to " "a Python object".format(attr_value)
        )


def load_variables_to_tf_graph(
    g  # type: graph.Graph
):
    """
...
def make_const(
  ...
  """
    ...
    ret.set_outputs_from_pairs([(dtype, tf.TensorShape(shape))])
    return ret
