...
# First Party
from smdistributed.modelparallel.tensorflow.graph_def_editor import (
    graph,
    node,
    reroute,
    tensor,
...
from smdistributed.modelparallel.tensorflow.graph_def_editor.select import TreeExpr

if sys.version >= "3":
    from typing import Tuple, Dict, Iterable, Union, Callable, Any


__all__ = ["change_batch_size", "fold_batch_norms"]
...
def change_batch_size(
  ...
  """
    input_nodes = [i.node if isinstance(i, tensor.Tensor) else i for i in inputs]
...
def _scale_weights(
  ...
  """
    if len(dims) != len(scale.shape):
        raise ValueError(
            "Target dimensions {} not compatible with shape of "
            "scale array {}".format(dims, scale)
        )
    if weights_node.op_type != "Const":
        raise TypeError("Unexpected op type {} for weights_node".format(weights_node.op_type))

    weights = weights_node.get_attr("value")
    for i in range(len(dims)):
        if scale.shape[i] != weights.shape[dims[i]]:
            raise ValueError(
                "Scale vector of shape {} can't be applied along "
...
def _add_scale_to_conv_weights(
    conv_node,  # type: node.Node
    weights_node,  # type: node.Node
    scale,  # type: np.ndarray
):
    # type: (...) -> None
  ...
  """
    ...
    if conv_node.op_type == "DepthwiseConv2dNative":
        ...
        raise ValueError("Unexpected op type {} for conv_node".format(conv_node.op_type))
    ...
    pattern = TreeExpr(
        ...
        inputs=(
            TreeExpr(
                ...
                alias="conv",
                inputs=(TreeExpr(), TreeExpr(op="Const", alias="weights")),
            ),
...
def _get_batch_norm_params(
  ...
  """
    ...
    def get_const_values():
        return (
            np.float64(batch_norm_node.inputs[ix].node.get_attr("value"))
    ...
    if "BatchNormWithGlobalNormalization" == batch_norm_node.op_type:
        ...
        variance_epsilon = np.float64(batch_norm_node.get_attr("variance_epsilon"))
        scale_after_normalization = bool(batch_norm_node.get_attr("scale_after_normalization"))
        ...
        raise TypeError(
            "Unexpected op type {} for fused batch norm".format(batch_norm_node.op_type)
        )
    return (mean, variance, beta, gamma, variance_epsilon, scale_after_normalization)
...
def _get_scale_and_offset(match_info):
  ...
  """
    ...
    (
        ...
        variance_epsilon,
        scale_after_normalization,
    ) = _get_batch_norm_params(batch_norm_node)
    ...
    if variance.shape != mean_shape or beta.shape != mean_shape or gamma.shape != mean_shape:
        raise ValueError(
            "Shapes {}, {}, {}, and {} for mean, variance, beta, "
            "and gamma don't all match"
            "".format(mean.shape, variance.shape, beta.shape, gamma.shape)
        )
...
def _replace_batch_norm_with_bias_add(
  ...
  """
    ...
    conv_node = match_info["conv"] if "conv" in match_info else match_info["conv0"]
    data_format = conv_node.get_attr("data_format") if conv_node.has_attr("data_format") else None

    # TODO(frreiss): Support non-32-bit offsets
    bias_offset_node = util.make_const(
        g, batch_norm_node.name + "_offset", np.float32(offset), uniquify_name=True
    )
    bias_add_node = g.add_node(batch_norm_node.name + "_bias_add", "BiasAdd", uniquify_name=True)
    ...
    bias_add_node.set_outputs_from_pairs(
        [(batch_norm_node.output(0).dtype, batch_norm_node.output(0).shape)]
    )
...
def fold_old_batch_norms(g):
  ...
  """
    ...
    pattern_1 = TreeExpr(
        ...
        inputs=(
            TreeExpr(
                op="Conv2D|DepthwiseConv2dNative",
                alias="conv",
                inputs=(TreeExpr(), TreeExpr(op="Const", alias="weights")),
            ),
            TreeExpr(op="Const"),
            ...
            TreeExpr(
                ...
                inputs=(
                    TreeExpr(
                        op="Conv2D",
                        alias="conv0",
                        inputs=(TreeExpr(), TreeExpr(op="Const", alias="weights0")),
                    ),
                    TreeExpr(
                        op="Conv2D",
                        alias="conv1",
                        inputs=(TreeExpr(), TreeExpr(op="Const", alias="weights1")),
                    ),
...
def fold_batch_norms_up(g):
  ...
  """
                    ...
                    TreeExpr(
                        ...
                        inputs=(
                            TreeExpr(
                                op="Mul",
                                alias="mul",
                                inputs=(TreeExpr(), TreeExpr(op="Const", alias="mul_values")),
                            ),
    ...
    """
        ...
        min_values = (6.0 / scale).astype(target_np_type)
        ...
        reroute.reroute_ts(min_node.output(0), relu6_op.output(0), cannot_modify=[min_node])
