...
from six import StringIO, iteritems, iterkeys, string_types

# First Party
from smdistributed.modelparallel.tensorflow.graph_def_editor import select, subgraph, util
...
from smdistributed.modelparallel.tensorflow.graph_def_editor.variable import Variable
...
def copy_op_handler(
  ...
  """
    ...
    op_ = info.graph_.add_node_from_node_def(node_def_, set_inputs=False, set_control_inputs=False)
...
class TransformerInfo(object):
    ...
    def _get_transformed_map(self, top):
        ...
        if isinstance(top, Node):
            ...
            raise TypeError("Expected a gde.Tensor or a gde.Node, got a {}".format(type(top)))
  ...
  """
    ...
    def _connect_control_inputs(self, info):
        ...
        for op in info.sgv.ops:
            ...
            control_inputs_ = [
                self.transform_control_input_handler(info, ci) for ci in op.control_inputs
            ]
        ...
        if t in info.sgv_inputs_set:
            ...
            if consumer_op.op_type == "Merge":
                ...
                tmp_t_ = util.make_placeholder_from_tensor(
                    info.graph_, t, scope=info.scope_, prefix="geph_tmp"
                )
...
def copy_with_input_replacements(
    sgv, replacement_ts, dst_graph=None, dst_scope="", src_scope="", reuse_dst_scope=False
):
...
def graph_replace(target_ts, replacement_ts, dst_scope="", src_scope="", reuse_dst_scope=False):
  ...
  """
    ...
    ops = select.get_walks_intersection_ops(
        list(iterkeys(replacement_ts)), flatten_target_ts, control_ios=control_ios
    )
    if not ops:
