...
if sys.version >= "3":
    from smdistributed.modelparallel.tensorflow.graph_def_editor import graph
  ...
  """
    ...
    """
        if g.has_passthrough_saver:
            ...
            raise ValueError(
                "Attempted to add a variable to Graph '{}', which has "
    ...
    def __repr__(self):
        return "Var[name={}, init={}, val={}, " "snap={}, t={}]".format(
            self.name,
            self._initializer_name,
            self._initial_value_name,
            self._snapshot_name,
            self._trainable,
        )

    def is_same_variable(
        self, other  # type: Variable
    ):
        """
    ...
    def validate(
        self, allow_duplicate=False  # type: bool
    ):
        """
        ...
        if self._variable_name in self.graph.variable_names:
            ...
            if other_var is not self:
                if not self.is_same_variable(other_var):
                    raise ValueError(
                        "Existing '{}' in graph conflicts with this one "
                        "({} != {})".format(self._variable_name, repr(self), repr(other_var))
                    )
                elif not allow_duplicate:
                    raise ValueError(
                        "Graph already has a variable called '{}'".format(self._variable_name)
                    )
        # self._initializer_name should reference a node. Other names should
        # reference tensors.
        if not self.graph.contains_node(self._initializer_name):
            raise ValueError(
                "Initializer name '{}' does not correspond to any "
                "node in graph".format(self._initializer_name)
            )
        _ = self.graph.get_tensor_by_name(
            self._initial_value_name, "Invalid initial value name '{}': {}"
        )
        _ = self.graph.get_tensor_by_name(self._snapshot_name, "Invalid snapshot name '{}': {}")
