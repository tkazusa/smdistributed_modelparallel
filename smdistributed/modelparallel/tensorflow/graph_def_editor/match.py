...
# First Party
from smdistributed.modelparallel.tensorflow.graph_def_editor import node, select

__all__ = ["op_type", "OpMatcher"]
...
class OpMatcher(object):
    ...
    def __call__(self, op):
        ...
        if self.control_input_op_matches is not None:
            if len(op.control_inputs) != len(self.control_input_op_matches):
                return False
            for cinput_op, cinput_op_match in zip(op.control_inputs, self.control_input_op_matches):
        ...
        if self.output_op_matches is not None:
            if len(op.outputs) != len(self.output_op_matches):
                return False
            for output_t, output_op_matches in zip(op.outputs, self.output_op_matches):
                if output_op_matches is None:
                    continue
                if len(output_t.consumers()) != len(output_op_matches):
                    return False
                for consumer_op, consumer_op_match in zip(output_t.consumers(), output_op_matches):
    ...
    def output_ops(self, *args):
        ...
        for consumer_op_matches in args:
            ...
            consumer_op_matches = [
                _make_graph_match(consumer_op_match) for consumer_op_match in consumer_op_matches
            ]
