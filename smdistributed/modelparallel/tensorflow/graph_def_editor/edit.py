...
# First Party
from smdistributed.modelparallel.tensorflow.graph_def_editor import reroute, select, subgraph, util
...
def detach_inputs(sgv, control_inputs=False):
  ...
  """
    ...
    # New code:
    input_placeholders = [
        util.make_placeholder(
            sgv.graph, util.placeholder_name(input_t), input_t.dtype, input_t.shape
        ).output(0)
        for input_t in sgv.inputs
    ]
...
def detach_outputs(sgv, control_outputs=None):
  ...
  """
    ...
    sgv_ = sgv.remap_outputs(
        [output_id for output_id, output_t in enumerate(sgv.outputs) if output_t.consumers()]
    )
    ...
    consumers_sgv = consumers_sgv.remap_inputs(
        [
            input_id
            for input_id, input_t in enumerate(consumers_sgv.inputs)
