...
# First Party
from smdistributed.modelparallel.tensorflow.graph_def_editor import graph, tensor, util
  ...
  """
    ...
    """
        if index < 0 or index >= len(self._inputs):
            raise IndexError(
                "Received input index {}, but node has {} "
    ...
    * Yes, it's supposed to be spelled "collocate"/"collocation", but the
    ...
    """
        for s in value:
            if not self._graph.contains_node(s):
                raise ValueError("Graph does not contain a node with name '{}'".format(s))
    ...
    """
        ...
        if validate and not self._graph.contains_node(head_node_name):
            raise ValueError("Graph does not contain a node with name '{}'".format(head_node_name))
        if head_node_name in self._colocation_groups:
            raise ValueError("Already have colocation group with '{}'".format(head_node_name))
    ...
    """
        if target is None:
            target = node_def_pb2.NodeDef()  # tf.NodeDef()
        ...
        for (attr_name, attr_value) in self._attributes:
            ...
            target.attr[attr_name].CopyFrom(util.python_type_to_attr_value(attr_value))
        if len(self._colocation_groups) > 0:
            ...
            transformed_names = [_COLOCATION_PREFIX + name for name in self._colocation_groups]
    ...
    """
        ...
        if 0 == len(matches):
            raise ValueError(
                "Node {} does not have an attribute " "under key '{}'".format(self, key)
            )
        elif len(matches) > 1:
            raise ValueError(
                "Node {} has more than one attribute " "under key '{}'".format(self, key)
            )
        ret = matches[0]
        if isinstance(ret, attr_value_pb2.AttrValue):
    ...
    """
        attr_keys = [att[0] for att in self._attributes]
        return key in attr_keys
    ...
    """
        if self._outputs is None:
            ...
            if len(new_shapes) != len(self._outputs):
                raise ValueError(
                    "Attempted to put in place {} output shapes, "
                    "but node has {} outputs.".format(len(new_shapes), len(self._outputs))
                )
    ...
    """
        if key == _COLOCATION_ATTR_NAME:
            ...
            if len(self.colocation_groups) == 0:
                raise ValueError(
                    "Tried to replace special '{}' attribute when the "
    ...
    """
        if self._outputs is not None and len(new_outputs) != len(self._outputs):
            ...
            raise NotImplementedError(
                "Attempted to change number of output tensors "
                "on node {} from {} to {}. Changing the "
                "number of output tensors is not currently "
                "supported.".format(self.name, len(self._outputs), len(new_outputs))
            )
        elif self._outputs is None:
            self._outputs = [tensor.Tensor(self, i, None, None) for i in range(len(new_outputs))]
    ...
    """
        if self.op_type == "Assign":
            ...
            with temp_graph.as_default():
                input_placeholders = [
                    tf.placeholder(shape=t.shape, dtype=t.dtype) for t in self._inputs
                ]
                # See the docs for tf.Operation for important notes about the semantics
                # of each arg to the following constructor.
                dummy_op = tf.Operation(self.to_node_def(), temp_graph, inputs=input_placeholders)
                self.set_outputs_from_pairs([(o.dtype, o.shape) for o in dummy_op.outputs])
...
def _decode_inputs(
  ...
  """
    ...
    # "node_name:0".
    input_names = [_canonicalize_output_name(n) for n in inputs if not n.startswith("^")]
    input_tensors = []
  ...
  Returns a list of node names that comprise the group."""
    if isinstance(value, attr_value_pb2.AttrValue):
        ...
        if value.list.s is None:
            raise ValueError(
                "Tried to set special '{}' attribute using "
        ...
        raise ValueError(
            "Tried to set special '{}' attribute with a type "
            "other than list or tuple. Type is '{}' and value "
            "is '{}'".format(_COLOCATION_ATTR_NAME, type(value), str(value))
        )
    ret = []
    for elem in value:
        ...
        if elem.startswith(_COLOCATION_PREFIX):
  ...
  """
    ...
    if isinstance(value, AttrValue):
        ...
        raise ValueError(
            ...
            "is '{}'".format(_OUTPUT_SHAPES_ATTR_NAME, type(value), str(value))
        )
