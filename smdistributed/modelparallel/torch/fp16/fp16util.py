...
from smdistributed.modelparallel.torch.core import tp_rank, tp_size
...
def prep_param_lists(model, flat_master=False):
    ...
    .. warning::
        Currently, if ``flat_master=True``, all the model's parameters must be the same type.  If the model has parameters of different types, use ``flat_master=False``, or use :class:`Bit16_Optimizer`.
            ...
            if model.grad is not None:
                if master.grad is None:
                    if params_have_main_grad:
                        ...
                        # This path is deprecated and will harm performance
...
def get_tp_merged_fp32_from_fp16_param_groups(optimizer, cpu_fp32_from_fp16_groups):
    ...
    param_id_to_index_tp_group = allgather(param_id_to_index, CommGroup.TP_GROUP)
    ...
    fp32_from_fp16_paramid_groups_tp_group = allgather(
        fp32_from_fp16_paramid_groups, CommGroup.TP_GROUP
    )
    fp32_from_fp16_param_groups_tp_group = allgather(cpu_fp32_from_fp16_groups, CommGroup.TP_GROUP)
    ...
    if tp_rank() == 0:
        ...
        broadcast(master_distribution_axis_tp_rank_0, CommGroup.TP_GROUP)
    else:
        master_distribution_axis_tp_rank_0 = recv_from(0, RankType.TP_RANK)
...
def get_pp_merged_fp32_from_fp16_param_groups(
    ...
    pp_group_fp32_from_fp16_groups = allgather(fp32_from_fp16_groups, CommGroup.PP_GROUP)
    if param_name_groups is not None:
        ...
        pp_index_to_param_name_groups = allgather(index_to_param_name_groups, CommGroup.PP_GROUP)
    else:
