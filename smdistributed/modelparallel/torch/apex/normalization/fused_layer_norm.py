...
from smdistributed.modelparallel.torch.apex._autocast_utils import _cast_if_autocast_enabled
...
class FusedLayerNorm(torch.nn.Module):
    ...
    Examples::
        ...
        >>> m = smdistributed.modelparallel.torch.apex.normalization.FusedLayerNorm(input.size()[1:])
        >>> # Without Learnable Parameters
        >>> m = smdistributed.modelparallel.torch.apex.normalization.FusedLayerNorm(input.size()[1:], elementwise_affine=False)
        >>> # Normalize over last two dimensions
        >>> m = smdistributed.modelparallel.torch.apex.normalization.FusedLayerNorm([10, 10])
        >>> # Normalize over last dimension of size 10
        >>> m = smdistributed.modelparallel.torch.apex.normalization.FusedLayerNorm(10)
