...
class FusedNovoGrad(torch.optim.Optimizer):
    ...
    :class:`smdistributed.modelparallel.torch.apex.optimizers.FusedNovoGrad`'s usage is identical to any Pytorch optimizer::

        opt = smdistributed.modelparallel.torch.apex.optimizers.FusedNovoGrad(model.parameters(), lr = ....)
        ...
        opt.step()

    :class:`smdistributed.modelparallel.torch.apex.optimizers.FusedNovoGrad` may be used with or without Amp.  If you wish to use :class:`FusedNovoGrad` with Amp,
    ...
    def __init__(self, params, lr=1e-3, bias_correction=True,
        ...
        if multi_tensor_applier.available:
            ...
            raise RuntimeError('smdistributed.modelparallel.torch.apex.optimizers.FusedNovoGrad requires cuda extensions')
