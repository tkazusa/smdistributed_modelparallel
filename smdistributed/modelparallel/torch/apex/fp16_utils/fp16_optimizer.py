...
class FP16_Optimizer(object):
    def __init__(self, 
        ...
        self.use_multi_tensor_applier = True
            ...
            for i, param in enumerate(param_group['params']):
                if param.requires_grad:
                    if param.type() in self.supported_tensor_types:
                        if param.type() == "torch.cuda.BFloat16Tensor":
                            self.use_multi_tensor_applier = False
    ...
    def _master_params_to_model_params(self):
        if multi_tensor_applier.available and self.use_multi_tensor_applier:
